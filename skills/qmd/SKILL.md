...
license: MIT
compatibility: Requires qmd CLI or MCP server. Install via `go install ./cmd/qmd@latest` or build from source.
metadata:
  author: tobi
  version: "1.1.1"
...
!`qmd status 2>/dev/null || echo "Not installed. Run: go install ./cmd/qmd@latest"`
